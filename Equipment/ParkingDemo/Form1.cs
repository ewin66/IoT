...
namespace ParkingDemo
{
    ...
    {   
        ...
        private System.Windows.Forms.Timer timerUpdateParkingOccupy;

        Business.Parking bllParking;
        public Form1()
        {
            bllParking = new Business.Parking();
            timerUpdateParkingOccupy = new System.Windows.Forms.Timer();
            timerUpdateParkingOccupy.Interval = 60000;
            timerUpdateParkingOccupy.Tick += TimerUpdateParkingOccupy_Tick;
            InitializeComponent();
        }

        private void TimerUpdateParkingOccupy_Tick(object sender, EventArgs e)
        {
            bllParking.UpdateParkingOccupy("SMDS001");
        }
                ...
                if (severPorttextBox.Text != "")
                {
                    if (startListenbutton.Text == "Start  Listen")
                    {
                        ...
                        if (portNum > 0 && portNum < 65535)
                        {
                            ...
                            timerUpdateParkingOccupy.Start();
                        }
                        else
                        {
            ...
            try
            {
                ...
                this.Invoke((EventHandler)delegate
                {
                    ...
                    if (pk is SensorHBeat)
                    {
                        ...
                        voltage = (Math.Round((decimal)hb.VOLT / 10, 2)).ToString() ;
                        ...
                        if (carState == 0x01)
                        {
                            ...
                            bllParking.UpdateParking(wpsdid, RSSI, voltage, false);
                        }
                        else
                        {
                            if (ckBShow.Checked)
                            ...
                            bllParking.UpdateParking(wpsdid, RSSI, voltage, true);
                        }
                    ...
                    else if (pk is SensorDetect)
                    {
                        ...
                        if (carState == 0x01)
                        {
                            ...
                            //parking.SetParkingState(wpsdid,false);
                            bllParking.UpdateParking(wpsdid, false);
                        }
                        else
                        {
                            if (ckBShow.Checked)
                            ...
                            //parking.SetParkingState(wpsdid,true);
                            bllParking.UpdateParking(wpsdid, true);
                        }
                    }
                    #endregion
                });
        ...
        {
            groupBox1.Text = "Raw Data - Last Received Time:"+System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            string restr = "";
            if (text != null)
            {
            ...
            if (source)
            {
                ...
                if (ckBShow.Checked)
                {
                    richTextBox1.AppendText(System.DateTime.Now.ToString() + "[Send]:  " + restr + "\n");
                }
            }
        }
